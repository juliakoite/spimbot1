...
main:
		...
		or 		$t4, $t4, 		FEEDBACK_INT_MASK           # enable bonk interrupt
		...
		lw     $v0, TIMER($0)      
  		...
  		li     $a0, 10                    # set spimbot velocity to 10
	...
	#solve puzzle before or after get slabs? 
...
#BASIC MOVEMENT: 
	...
	# 	la $t0, state
	# 	move $a1, $t0
	# 	jal build_state #CALLING BUILD STATE w/ current feedback & State address
	# 	#need state address
	# 	la $t0, state
	# 	la $t1, words
	# 	move $a0, $t0
	# 	move $a1, $t1
	# 	#LOADING STATE& AND WORDS INTO
	# 	jal find_matching_word
	# 	#NEED TO SAVE GUESS
	# 	move $s2, $v0 #GUESS
	# 	#NEED TO LOAD &feedbacks[i] INTO PUZZLE_FEEDBACK*
	# 	la $t5, feedbacks
	# 	li $t6, 16
	# 	mul $t7, $t6, $s0 #find address of feedbacks[i]
	# 	add $t8, $t5, $t7
	# 	sw $t8, PUZZLE_FEEDBACK
	...
	# 		#while feedback int is equal ot 0 we wait
	# 		beq $t1, $0, wait
	# 		move $a0, $t8
	# 		sw $0, 0($t0)
	# 		jal is_solved
...
la $t5, has_bonked
lb $t6, 0($t5)
#check if bonk is from locked slab??

beq $t6, $zero, no_bonk
li $a0, 180
sw $a0, ANGLE($zero)
li $a1, 0
sw $a1, ANGLE_CONTROL($zero)
li     $a0, 10                 
sw     $a0, VELOCITY($zero)
...
#find nearest slab given address
nearest_slab:
#make bot follow the slab

#movement sorta works kinda movement is just continuous and i have to work on dealing w interrupts. 
# move_west:
# sub $sp, $sp, 4
# sw $ra, 0($sp)
...
#calculates nearest slab from data
